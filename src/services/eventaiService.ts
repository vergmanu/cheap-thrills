...
function formatSpecial(s: EventAISpecial): string {
  let description = s.item;
  if (s.price !== null) description += ` $${s.price}`;
  if (s.discount !== null) description += ` (${s.discount})`;
  return description;
}
